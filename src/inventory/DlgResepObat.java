  ...
  isForm();                
    ...
    private void ppLembarObat1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppLembarObat1ActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            param.put("peresep",NmDokter.getText());
            param.put("noresep",NoResep.getText());
            param.put("tgl_lahir",Sequel.cariIsi("select tgl_lahir from pasien where no_rkm_medis=?",TNoRm.getText()));//edit-novan
            param.put("sip",Sequel.cariIsi("select no_ijn_praktek from dokter where kd_dokter=?",KdDokter.getText()));//edit-novan

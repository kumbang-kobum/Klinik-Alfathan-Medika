...
public final class DlgKasirRalan extends javax.swing.JDialog {
    ...
    private String aktifkanparsial="no",kamar_inap_kasir_ralan=Sequel.cariIsi("select set_jam_minimal.kamar_inap_kasir_ralan from set_jam_minimal"),caripenjab="",filter="no",bangsal=Sequel.cariIsi("select set_lokasi.kd_bangsal from set_lokasi limit 1"),nonota="",
            ...
            namadokter="",namapoli="",order="reg_periksa.no_reg asc",
    ...
    public DlgKasirRalan(java.awt.Frame parent, boolean modal) {
        ...
        tabModekasir=new DefaultTableModel(null,new String[]{
            ...
            "Jam","No.Reg","Status Bayar","Stts Poli","Kd PJ","Kd Poli","No.Telp Pasien","No SEP"}){ //edit-novan Penambahan no SEP
        ...
        for (i = 0; i < 21; i++) { //edit-novan Penambahan Kolom No SEP ganti jadi 21
            ...
            if(i==0){
                ...
                column.setPreferredWidth(130); //edit-novan Penambahan Kolom No SEP
            }
        }
        try {
    ...
    private void initComponents() {
        ...
        TglSakit1.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "27-08-2023" }));
        ...
        TglSakit2.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "27-08-2023" }));
...
private void BtnBatalActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnBatalActionPerformed
        if(tbKasirRalan.getSelectedRow()>-1){
            kirimwa.sendWaBatal(tbKasirRalan.getValueAt(tbKasirRalan.getSelectedRow(),1).toString(),tbKasirRalan.getValueAt(tbKasirRalan.getSelectedRow(),2).toString(),tbKasirRalan.getValueAt(tbKasirRalan.getSelectedRow(),3).toString(),tbKasirRalan.getValueAt(tbKasirRalan.getSelectedRow(),4).toString(),tbKasirRalan.getValueAt(tbKasirRalan.getSelectedRow(),11).toString(),tbKasirRalan.getValueAt(tbKasirRalan.getSelectedRow(),12).toString());
...
private void MnDataPemberianObatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnDataPemberianObatActionPerformed
    ...
    private void ppRiwayatBtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppRiwayatBtnPrintActionPerformed
        if(tabModekasir.getRowCount()==0){
            ...
            resume.setNoRm(TNoRMCari.getText(),TPasienCari.getText(),TNoRwCari.getText()); //edit-novan auto no rawat di riwayat perawatan
    ...
    private void ppRiwayat1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppRiwayat1ActionPerformed
        if(tabModekasir2.getRowCount()==0){
            ...
            if(tbKasirRalan2.getSelectedRow()!= -1){
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                RMRiwayatPerawatan resume=new RMRiwayatPerawatan(null,true);
                resume.setNoRm(tbKasirRalan2.getValueAt(tbKasirRalan2.getSelectedRow(),2).toString(),tbKasirRalan2.getValueAt(tbKasirRalan2.getSelectedRow(),3).toString(),tbKasirRalan2.getValueAt(tbKasirRalan2.getSelectedRow(),11).toString()); //edit-novan auto no rawat di riwayat perawatan
                resume.setSize(internalFrame1.getWidth()-20,internalFrame1.getHeight()-20);
                resume.setLocationRelativeTo(internalFrame1);
                resume.setVisible(true);
                this.setCursor(Cursor.getDefaultCursor());
    ...
    */
    ...
    private void tampilkasir() {     
        ...
        try{   
            ...
            pskasir=koneksi.prepareStatement("select reg_periksa.no_reg,reg_periksa.no_rawat,reg_periksa.tgl_registrasi,reg_periksa.jam_reg,"+
                "reg_periksa.kd_dokter,dokter.nm_dokter,reg_periksa.no_rkm_medis,pasien.nm_pasien,poliklinik.nm_poli,"+
                "reg_periksa.p_jawab,reg_periksa.almt_pj,reg_periksa.hubunganpj,reg_periksa.biaya_reg,reg_periksa.stts,penjab.png_jawab,concat(reg_periksa.umurdaftar,' ',reg_periksa.sttsumur)as umur, "+
                "reg_periksa.status_bayar,reg_periksa.status_poli,reg_periksa.kd_pj,reg_periksa.kd_poli,pasien.no_tlp,bridging_sep.no_sep "+ //edit-novan Penambahan No SEP, bridging_sep.no_sep
                "from reg_periksa inner join dokter on reg_periksa.kd_dokter=dokter.kd_dokter inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                "inner join poliklinik on reg_periksa.kd_poli=poliklinik.kd_poli inner join penjab on reg_periksa.kd_pj=penjab.kd_pj left join bridging_sep on bridging_sep.no_rawat=reg_periksa.no_rawat where "+ //edit-novan Penambahan No SEP, left join bridging_sep on bridging_sep.no_rawat=reg_periksa.no_rawat
            ...
            try{
                ...
                while(rskasir.next()){
                    tabModekasir.addRow(new String[] {
                        ...
                        rskasir.getString("kd_pj"),rskasir.getString("kd_poli"),rskasir.getString("no_tlp"),rskasir.getString("no_sep") //edit-novan Penambahan No SEP, rskasir.getString("no_sep")
                    });
                }                
            } catch(Exception e){
    ...
    public void isCek(){
        ...
        MnBatal.setEnabled(akses.getpembatalan_periksa_dokter()); //edit-novan Disable batal periksa dari akun dokter
        ...
        MnPenjab.setEnabled(akses.getregistrasi()); //edit-novan mengubah hak akses jenis bayar hanya dari resepsionis
        ...
        if(akses.getkode().equals("Admin Utama")){
            ...
            if(kamar_inap_kasir_ralan.equals("Yes")){
                ...
                MnKamarInap.setEnabled(akses.getregistrasi()); //edit-novan mengubah hak akses Kamar Inap hanya dari resepsionis
                MnKamarInap1.setEnabled(akses.getregistrasi()); //edit-novan mengubah hak akses Kamar Inap hanya dari resepsionis
